...
const StatCard = ({ title, value, icon: Icon, color }) => (
  <div
    className={`bg-gray-800/80 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50 shadow-2xl transform hover:scale-105 transition-transform duration-300 ${color}`}
  >
...
const StatusBadge = ({ status }) => {
  ...
  switch (status?.toLowerCase()) {
    case "confirmed":
      return (
        <span
          className={`${baseStyle} bg-green-500/20 text-green-400 ring-1 ring-green-400/30`}
        >
          {status}
        </span>
      );
    case "pending":
      return (
        <span
          className={`${baseStyle} bg-yellow-500/20 text-yellow-400 ring-1 ring-yellow-400/30`}
        >
          {status}
        </span>
      );
    case "cancelled":
      return (
        <span
          className={`${baseStyle} bg-red-500/20 text-red-400 ring-1 ring-red-400/30`}
        >
          {status}
        </span>
      );
    case "delivered":
      return (
        <span
          className={`${baseStyle} bg-blue-500/20 text-blue-400 ring-1 ring-blue-400/30`}
        >
          {status}
        </span>
      );
    default:
      return (
        <span
          className={`${baseStyle} bg-gray-500/20 text-gray-400 ring-1 ring-gray-400/30`}
        >
          {status || "Unknown"}
        </span>
      );
...
const Dashboard = () => {
  ...
  const adminName =
    JSON.parse(localStorage.getItem("admin"))?.name?.split(" ")[0] || "Admin";
  ...
  return (
    <div className="p-0">
      ...
      <p className="text-gray-400 mb-8">
        Hello {adminName}! Here's your restaurant's performance summary.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 mb-10">
        ...
        <StatCard
          ...
          icon={TrendingUp}
          color="bg-gradient-to-br from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500"
        />
      </div>
